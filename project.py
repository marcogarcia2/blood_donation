...
class BloodDonationApp:
    ...
    def create_widgets(self):
        ...
        ttk.Button(control_frame, text="Carregar Grafo", command=self.carregar_grafo).grid(row=0, column=0, padx=5, pady=5)
        
        # Botão para selecionar origem aleatória
        ttk.Button(control_frame, text="Origem Aleatória", command=self.origem_aleatoria).grid(row=0, column=1, padx=5, pady=5)
        
        # Combobox para tipo sanguíneo
        ttk.Label(control_frame, text="Tipo Sanguíneo:").grid(row=0, column=2, padx=5, pady=5)
        tipos = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']
        ttk.Combobox(control_frame, textvariable=self.tipo_sanguineo, values=tipos, state="readonly").grid(row=0, column=3, padx=5, pady=5)
        
        # Radio buttons para algoritmo
        ttk.Label(control_frame, text="Algoritmo:").grid(row=0, column=4, padx=5, pady=5)
        ttk.Radiobutton(control_frame, text="A*", variable=self.algoritmo, value="A*").grid(row=0, column=5, padx=5, pady=5)
        ttk.Radiobutton(control_frame, text="BFS", variable=self.algoritmo, value="BFS").grid(row=0, column=6, padx=5, pady=5)
        
        # Botão para executar busca
        ttk.Button(control_frame, text="Encontrar Rota", command=self.encontrar_rota).grid(row=0, column=7, padx=5, pady=5)
    ...
    def carregar_grafo(self):
        try:
            self.grafo = Graph("sao_carlos.graphml")
